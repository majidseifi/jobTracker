...
const VALID_STATUSES = ['applied', 'interviewing', 'offer', 'rejected', 'accepted', 'withdrawn'];
...
function validateApplication(data, isUpdate = false) {
  ...
  if (data.rating !== undefined && data.rating !== null) {
    if (typeof data.rating !== 'number' || data.rating < 0 || data.rating > 100) {
  ...
  if (data.resumeVersion !== undefined && data.resumeVersion !== null) {
    if (typeof data.resumeVersion !== 'string' || data.resumeVersion.length > 50) {
  ...
  if (data.jobDescription !== undefined && data.jobDescription !== null) {
    if (typeof data.jobDescription !== 'string' || data.jobDescription.length > 10000) {
  ...
  if (data.coverLetter !== undefined && data.coverLetter !== null) {
    if (typeof data.coverLetter !== 'string' || data.coverLetter.length > 10000) {
    ...
    if (!Array.isArray(data.interviews)) {
      ...
      data.interviews.forEach((interview, index) => {
        if (!interview.date || !validateDate(interview.date)) {
          errors.push(`Interview ${index + 1}: Date is required and must be valid`);
        }
        if (interview.type && !VALID_INTERVIEW_TYPES.includes(interview.type)) {
          errors.push(`Interview ${index + 1}: Type must be one of: ${VALID_INTERVIEW_TYPES.join(', ')}`);
        }
      });
    }
  }
...
function validateInterview(data) {
  ...
  if (!data.date || !validateDate(data.date)) {
    errors.push('Interview date is required and must be a valid ISO date string');
  }

  if (data.type && !VALID_INTERVIEW_TYPES.includes(data.type)) {
    errors.push(`Interview type must be one of: ${VALID_INTERVIEW_TYPES.join(', ')}`);
  }
